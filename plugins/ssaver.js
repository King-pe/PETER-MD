...
// Auto-comment on viewed statuses using AI summary
smd({ on: "status" }, async (message) => {
  try {
    if (message.fromMe) return;
    // Build a prompt from caption or type
    const caption = message.text || message.caption || "";
    const kind = message.mtype || (message.message ? Object.keys(message.message)[0] : "status");
    const base = caption && caption.length > 2 ? caption.slice(0, 300) : `A WhatsApp ${kind} without caption`;
    const prompt = `Write a short, friendly, relevant one-liner comment for this status: "${base}". Avoid emojis if not needed.`;
    const reply = await aiResponce(message, "gpt", prompt);
    if (reply && typeof reply === "string" && reply.length > 0) {
      await message.reply(reply.trim().slice(0, 400));
    }
  } catch (e) {
    console.log("status auto-comment error:", e);
  }
});
